...
def recommendation_by_product(productId, df, df_products, algorithm, top_n=5):
    df_select = df[df['ma_san_pham'] == productId]
    df_select = df_select.set_index('ma_khach_hang')
    df_score = pd.DataFrame(df['ma_san_pham'].unique(), columns=['ma_san_pham'])
    df_score['EstimateScore'] = df_score['ma_san_pham'].apply(lambda x: algorithm.predict(df_select.index[0], x).est)
    df_score = df_score.sort_values(by=['EstimateScore'], ascending=False)
    df_score = df_score[df_score['ma_san_pham'] != productId]
    df2_selected = df_products[['ma_san_pham', 'ten_san_pham', 'gia_ban', 'gia_goc', 'mo_ta']]
    df_score = df_score.merge(df2_selected, on='ma_san_pham', how='left')
    df_score_with_images = pd.merge(df_score, df_hinh[['ma_san_pham', 'hinh_anh']], on='ma_san_pham', how='left')
...
# Tải dữ liệu
df_hinh = pd.read_csv('hinh_anh.csv')
...
df_reviews = pd.read_csv('Danh_gia_final2.csv')
...
# Menu ở sidebar
menu = ["Business Objective","Build Project", "Hiển thị chart", "Hệ thống gợi ý","Admin"]
...
# Nội dung mô tả
    ...
    with tab1:
        ...
        selected_product = st.selectbox(
            ...
            key="product_selectbox"  # Unique key for the product selectbox
        )
        ...
        selected_customer = st.selectbox(
            ...
            format_func=lambda x: x[0],
            key="customer_selectbox"  # Unique key for the customer selectbox
        )
        ...
        if selected_customer:
            ...
            recommendations = get_recommendations(df_products, recent_product['ma_san_pham'], cosine_sim_new, df_hinh, nums=5)
            if not recommendations.empty:
    ...
    with tab2:
        ...
        if not input_customer_name and not input_customer_id:
            ...
            selected_customer = st.selectbox(
                ...
                options=customer_options,
                format_func=lambda x: x[0],
                key=f"customer_selectbox_{input_customer_name or input_customer_id}"  # Unique key for customer selectbox
            )
        else:
